...
The manifest fields generated by this builder are `publisher`, `entry`, `sources`, `packages`, `files`, `services`, and `ports`. `services` maps names to rendered immutable unit paths. `ports` uses NixOS's `allowedUDPPorts` and `allowedTCPPorts` lists. Tailscale requests UDP 41641. Empty contributions need no payload. Native dependencies remain ordinary Nix references through packages, named files, services, and source inputs. Korri has no second plugin dependency graph. The current one-service limit is retained.
