...
Build outputs are `packages.<system>.korri-plugin-host` and `packages.<system>.korri-tailscale`. The [owner-triggered publication workflow](PUBLICATION.md) checks both Linux architectures and prepares complete Tailscale archives plus a separate small catalog. Live publication and the official HTTPS destination still need owner approval. General core binary caching is separate; Releases is not a Nix substituter.
...
The Nix devshell and package check both supply `KORRI_TEST_CURL`. The real local TLS tests fail if it is absent; none silently return. One explicitly ignored test is a subprocess entrypoint invoked by the ambient-credentials test. The build-side publisher integration tests run explicitly through `nix run .#korri-publisher-check`, which supplies the dynamically built Tailscale package, locked Nix and current system. They require a writable build-machine store; regular package and HTTPS tests remain sandboxed. Metadata CLI and workflow checks are described in [PUBLICATION.md](PUBLICATION.md).
