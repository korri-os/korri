...
`lib.<system>.mkPlugin { publisher; source; plugin; }` is exported by the plugin-host Nix composition. `source` is a directory whose required entry is `plugin.ts`; imported TS/JS/JSON modules and package metadata live below that root. The source package is a closed regular-file tree: the root, directories, and files must not be symlinks. Authors must copy package sources into that tree instead of relying on build-machine `node_modules` links. The builder admits only TS, JS, and JSON files, copies them, and generates a sorted manifest inventory. It caps the graph at 512 files, 4 MiB of content, 64 KiB of aggregate relative-path bytes, and 4096 aggregate path components. The path limits are deliberately below the host's admission budgets. A single source file is not accepted. `plugin` is a `plugin.nix` path or a function receiving `{ pkgs; }`. `publisher.namespace` comes from trusted publication composition, not TypeScript. The lower-level `builder.nix { pkgs; }` API permits a caller-owned package set. Rendering always imports that same `pkgs.path` and passes those `pkgs` to NixOS `eval-config.nix`.
