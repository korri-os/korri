...
The host installs previously unknown plugins without a NixOS update or a device build. It admits up to three native units named by one plugin declaration and refuses any packaged unit that the declaration did not name. It validates each shipped systemd unit and binds its exact authority to that plugin's approval. It accepts no installation scripts or TypeScript service directives.
...
The concrete example is `tests/fixtures/tailscale/plugin.nix`. Its `packages`, named `files`, and standard NixOS `services` produce the generated manifest. `files` are checked with `test -e` during the build. Service values can be standard NixOS service attributes or references to actual native `.service` files (paths, strings, or derivation outputs). Attribute values are rendered by NixOS, not a Korri template. Native file references pass through unchanged. Both reach the same runtime admission and approval boundary. The build context has no global locale, timezone or command-search environment from the CI system. Explicit service `path` is refused; use an admitted native `ExecStart` path. Environment directives stay in the rendered unit and enter the approval report and digest.

The manifest fields generated by this builder are `publisher`, `entry`, `sources`, `packages`, `files`, `services`, and `ports`. `services` maps names to rendered immutable unit paths. The declaration must request that exact set by name. `ports` uses NixOS's `allowedUDPPorts` and `allowedTCPPorts` lists. Tailscale requests UDP 41641. Empty contributions need no payload. Native dependencies remain ordinary Nix references through packages, named files, services, and source inputs. Korri has no second plugin dependency graph.
...
Install leaves the plugin disabled. Approval binds to the exact store path, the complete manifest-selected source graph (including any launch code), manifest bytes, explicit input provenance, declaration, base policy, and complete rendered unit. Nix realizes and verifies the package closure, but Korri approves, installs, enables, disables, rolls back, and removes only the selected plugin. Repository approval never includes the disposable staging/cache path. Two sources serving identical bytes have different approvals. There is no blanket `--yes` grant.
...
| `enable ID` | Enable the approved package and start its daemon, if any. |
| `disable ID` | Stop and clean up the daemon. Retain private data. |
| `remove ID` | The generic operator CLI fails closed because it has no authoritative product lifecycle policy. A product or release caller can construct `Host` with that policy to stop, clean up, release both selections, and reclaim unreachable software. |
| `remove ID --purge` | The same policy-bound operation can also ask systemd to delete the plugin's private state. |
...
If native cleanup fails, disable, removal and automatic rollback stop. The receipt and package roots remain available for inspection. Read the unit journal. Do not delete the roots to force success. A failed native cleanup can need separate administrator repair; the CLI does not pretend to reverse arbitrary native effects.

Removal then releases current, previous, and pending selection roots plus both acquisition-root spellings (`download` and an interrupted `download.new`) before it runs `nix store gc`. Nix deletes only paths unreachable from every GC root, so retained system generations, other plugin selections, and shared dependencies stay. A store-cleanup failure leaves the `Removed` receipt in place with no selection roots and reports the uninstall as incomplete. `restore-all` clears stale acquisition roots first, then retries that cleanup from the persisted removal intent without loading or re-enabling the plugin. Successful cleanup deletes the receipt last. Ordinary removal does not delete state directories, saves, settings, credentials, or identity data.

Every removal consults the authoritative `LifecyclePolicy` supplied when `Host` is constructed. There is no no-policy or default-policy production constructor. The generic operator CLI has no product or release authority, so it constructs a fail-closed host and cannot remove any plugin. A product or release caller can supply its agreed behavior view in memory; required-plugin refusal names that behavior, while an authoritative empty view permits ordinary removal. Requiredness is not a receipt field, plugin declaration, dependency graph, default-bundle inference, or persisted configuration.

`Host::review_release_update` returns every newly required plugin's existing full `Report` before any selection changes. `Host::apply_release_update` requires exact approvals for all of them, installs required additions before invoking the caller's release-selection operation, and rolls back those new selections if installation or release selection fails. Recommended optional reports are never installed by this transaction. Existing enabled, disabled, and removed choices remain unchanged. The transaction is operational and adds no persisted release or requiredness format.

## Installed game admission

Zero-service packages can carry the shipped `systems`, `families`, `runners`, `discovery`, `sessionControls`, `transports`, `providers`, `android`, and `config` data exports. The installer retains these records without translating them. Record interpretation belongs to the game registry. Inspection never calls `launch`; approval binds its exact source graph. Reports expose the manifest's `packages` and `files` fields. There is no Korri-level `requires` or `brings` field.

`Host::enabled_packages()` and the administrator-only `enabled-packages` command expose a consistent admission snapshot. They perform no receipt repair, package realization or callback effects. If Nix lacks a saved signature, verification can fetch signing metadata from the bound cache and retain it, as activation does. They reject changed approvals, revoked signing keys, unfinished selections, and inconsistent active roots. A cached signature still requires the full bound key. Verification holds the host lock and can be slow for large installed sets; a concurrent install makes the query fail as busy.

Each lifecycle operation manages exactly one plugin receipt. Native dependencies are retained by the selected package's Nix closure. Cross-plugin launcher/runtime references are resolved from the enabled declaration set; they are not installation or lifecycle dependencies. Native file keys must belong to the declaring package.

Recovery validates and restores each receipt independently. An invalid receipt is reported without creating an ordering relationship among other plugins. Korri does not infer dependency order from Nix closure edges.
...
`src/declaration.rs` consumes named exports `name`, optional `title` and `description`, `services`, and the existing data exports listed above. A declaration can name zero to three native units. Service names reference the generated manifest. Missing, extra, and repeated names fail by name; a fourth unit fails the three-unit bound. No `daemons` export or compatibility reader remains. The publisher namespace comes from verified packaging, not source. ID segments follow the grammar in `services/korrid/src/plugin.rs`, with a 64-byte bound.

`src/native_unit.rs` parses systemd logical lines, comments, quotes, C escapes, list resets, and the generated `[Install] WantedBy` metadata. Unsupported grammar fails closed. Service admission accepts the native identity, environment, socket attachment, command, host-path, runtime-directory, lifecycle, sandbox, capability, device, and named-credential directives used by the three current streaming-host units. Socket admission accepts the native `[Socket]` directives used by the certificate-control socket. The exact producer-derived fixture is under `tests/fixtures/streaming-host/`. Named users and groups become explicit approval requests; `User=root` still selects device-wide root policy. Every service command gets immutable-closure validation, except the exact `/run/wrappers/bin/<name>` form used by the host-owned KMS capability wrapper. Executable store outputs and paths inside store outputs are both accepted. A privileged `+` prefix is accepted only for immutable `ExecCondition`, `ExecStartPre`, and `ExecStopPost` commands and is named in the report. The warning states that the pre-start and post-stop helpers bypass the service sandbox with root authority. `Type` is `simple`, `notify`, or `exec`. Approved capabilities are `CAP_CHOWN`, `CAP_NET_ADMIN`, `CAP_NET_RAW`, `CAP_SETPCAP`, and `CAP_SYS_ADMIN`. Approved device requests are TUN, numbered DRM nodes, uinput, and uhid with `rw` access. An explicit `DevicePolicy=closed` remains closed even with `PrivateDevices=false`; an explicit `DevicePolicy=auto`, or the producer-grounded unrestricted Sunshine request with no narrower device list, selects `auto`. An explicit address-family restriction replaces the host default without adding families. The producer-grounded `korri-input-seat` runtime directory and its mode replace the host runtime default without losing the host-owned state directory. Unknown units, directives, address families, runtime directories, capabilities, and devices are refused by name, not dropped. Systemd specifiers and arbitrary environment expansion are refused. Native `${STATE_DIRECTORY}` and `${RUNTIME_DIRECTORY}` expansion stays owned by systemd.

Both selected immutable paths and resolved symlink targets must belong to the package's actual Nix closure. The host checks that commands resolve to regular executables. Being present elsewhere in `/nix/store` grants no authority. The host-owned `/run/wrappers/bin/<name>` exception is explicit in the unit and approval report. Each unit source remains bounded to 128 KiB and is read as a regular file. Its exact bytes and the effective hardening enter the approval digest with the manifest, which also binds the requested ports.

A declaration name becomes the actual systemd unit name, with `.service` added when the native source is a service and no suffix was supplied. Authors must use globally unique names; the host refuses any collision. `unit_configuration` shows every requested native unit, its exact source, and the applicable policy version before approval. That complete configuration enters the approval digest with the manifest and plugin identity. `src/unit.rs` activates the approved service and socket set as one plugin lifecycle. It records exact runtime ownership under `/run/korri-plugin-host/units`, refuses collisions with host or other-plugin units, checks the systemd-loaded fragment before starting, restoring, or stopping an owned name, starts every unit before opening ports, and removes every unit only after required service cleanup succeeds. Plugins cannot ship host drop-ins. Network administration and `CAP_SYS_ADMIN` grant substantial device authority, but only the exact plugin build whose approval names them receives it.

An explicit native `User=root` requests **device-wide root authority**. The report names that authority before approval. This is not a constrained capability profile: the daemon and all its lifecycle commands can change accounts, files, credentials, devices and network policy. The host retains private state/runtime directories, timeouts, restart and control-group stop policy, but does not apply the dynamic-user sandbox, capability drop, read-only system, hidden homes or privilege-escalation restrictions. Administrative OpenSSH needs account switching, writable account homes, PAM and PTYs. The native request, not a plugin ID or publisher, selects this policy. Every other plugin needs its own exact-build approval to request it. The selected policy version appears in each effective unit and in the approval digest. Policy v3 is a clean cut and changes existing approvals.
...
The GC roots use Nix's existing `/nix/var/nix/gcroots` convention. `image-seed.nix` uses the CLI's existing `seed` report and `unit-name` producer to build an `Enabled` receipt and active root at image build time, without starting a unit; its `storePaths` must be copied by the SD builder. This fragment is not yet consumed by any device image. `active` pins the committed package. `previous` pins the one retained prior selection. `pending` pins an uncommitted candidate. Runtime units live in systemd's `/run/systemd/system` directory and disappear on reboot.
...
`src/selection.rs` owns this bounded history and its durable roots. A changed update or source switch replaces previous with current. Re-selecting the same package, provenance and approval keeps previous unchanged. Enable and disable also retain previous. `restore ID` swaps the exact package, provenance and approval together, using current desired state. It rechecks both approvals and the candidate's current publisher authority. It never calls the importer or selects another plugin version. Signature verification may still consult the bound cache if Nix has lost signing metadata; rollback never downloads package contents.

The host roots pending before service effects. Until success, both committed selections and their approvals remain unchanged. One atomic receipt write commits current and previous together. The host then roots previous before replacing active, and finally removes pending. Recovery uses only that receipt, including after a crash between these steps. Failed starts roll back only after native cleanup succeeds. Failed cleanup retains the receipt and all transaction roots for repair. Removal records native cleanup first, then releases both selections before Nix store cleanup. The receipt remains as the recovery marker until reclamation succeeds. Successful recovery removes interrupted root temporaries as well.

The retention bound is two committed selections, plus one candidate during an update or failed transition. This is not a promise that Nix retains only two physical builds: exact closures and other installed packages can keep older outputs alive. A third successful update releases the displaced previous root. Rollback changes only the selected plugin. There is no automatic multi-plugin rollback or arbitrary-version substitution.
...
fields `id`, `package`, `files`, `entry`, and `sources`; it adds no plugin
manifest dependency fields. The snapshot contains only approved, enabled, committed selections.
...
the callback and auxiliary files come from its kind's package. The registry validates references against the enabled declarations. Lifecycle
selection does not create or enforce cross-plugin dependencies.
...
The mGBA package is self-contained: its closure carries the selected RetroArch
frontend, core, configuration evidence, and source graph. It is one independently
...
The current/previous verification record is in `LIFECYCLE-VALIDATION.md`. The current/previous VM gates cover offline swaps with exact source approvals, enabled and disabled intent, failed explicit restore, preservation across failed updates and reboot, and prior-root cleanup on removal. Run them only as part of the final integrated gate. Focused receipt/filesystem tests cover the atomic commit and each root repair boundary without a VM.
...
The plugins repository can also import `services/korrid/plugin-host/vm-test.nix` from pinned core. Its arguments are `pkgs`, `hostModule`, `hostPackage`, `korridPackage`, `tailscalePackage`, and `sshPackage`. `korridPackage` is the device's own brain: after the test enables a game plugin it starts korrid against the reviewed Linux route checkpoint and requires the game to be listed, which proves that an approved selection becomes a resolvable route. Supply the actual Tailscale package; the update fixture derives from that package's `plugin.ts` and generated manifest. Core's check supplies its test-only package instead. Both run the full cold-host, HTTPS, Headscale, lifecycle and failure tests.
