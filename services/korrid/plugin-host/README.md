...
`unit_configuration` shows every requested native unit, its exact source, and the applicable policy version before approval. That complete configuration enters the approval digest with the manifest and plugin identity. `src/unit.rs` still activates one service through the existing lifecycle seam. Multi-unit activation and cleanup belong to ticket 15; until that lands, inspection and disabled installation admit a multi-unit package but enable refuses it explicitly. Plugins cannot ship host drop-ins. Network administration and `CAP_SYS_ADMIN` grant substantial device authority, but only the exact plugin build whose approval names them receives it.
