...
fn native_decoder_sources_and_lazy_json_are_not_rewritten() {
    let graph = native_snapshot("import { TextEncoder, TextDecoder } from '@kayahr/text-encoding/no-encodings'; import load from './node_modules/@exodus/bytes/fallback/multi-byte.encodings.cjs'; export const name = 'native'; export function launch() { const data = new TextEncoder().encode('A😀'); const tables = load(); return { bytes: Array.from(data), text: new TextDecoder().decode(data), same: tables === load(), jis: Array.isArray(tables.jis0208) }; }", &[(PROBE, "@kayahr/text-encoding"), (PROBE, "@exodus/bytes")]);
    assert_eq!(
...
fn native_fast_check_and_pure_rand_execute_without_platform_polyfills() {
    let graph = native_snapshot("import * as fc from 'fast-check'; export const name = 'fast-check'; export function launch() { return { values: fc.sample(fc.constant(42), { seed: 1, numRuns: 3 }) }; }", &[(RETROARCH, "fast-check"), (RETROARCH, "pure-rand")]);
    assert_eq!(
        script::call_plugin_launch_snapshot(&graph, "null").unwrap(),
        r#"{"values":[42,42,42]}"#
    );
...
fn native_fast_check_require_branch_uses_its_nested_commonjs_scope() {
    let graph = native_snapshot_with_files("import sample from './sample.cjs'; export const name = 'commonjs-fast-check'; export function launch() { return { values: sample() }; }", &[(RETROARCH, "fast-check"), (RETROARCH, "pure-rand")], &[("sample.cjs", "const fc = require('fast-check'); module.exports = () => fc.sample(fc.constant(9), { seed: 1, numRuns: 2 });")]);
    assert_eq!(
        script::call_plugin_launch_snapshot(&graph, "null").unwrap(),
        r#"{"values":[9,9]}"#
    );
...
fn commonjs_cache_is_cleared_after_throw_but_successful_dependencies_stay_cached() {
    let graph = snapshot("import run from './load.cjs'; export const name = 'cache'; export function launch() { return run(); }", &[
...
fn commonjs_canonical_aliases_keep_their_parent_and_do_not_reopen_files() {
    ...
    fs::write(root.path().join("plugin.ts"), "import a from './alias.cjs'; import b from './real/value.cjs'; export const name = 'retained'; export function launch() { return { same: a === b, text: a() }; }").unwrap();
    fs::write(
...
fn a_guest_callback_cannot_reenter_an_evaluating_es_module() {
    ...
    for error in [
        ...
        script::call_plugin_launch_snapshot(&graph, "null").unwrap_err(),
    ] {
...
fn private_module_records_do_not_call_poisoned_prototype_setters() {
    let graph = snapshot("import load from './load.cjs'; export const name = 'records'; export function launch() { Object.defineProperty(Object.prototype, 'exports', { set() { throw new Error('poisoned'); } }); return load(); }", &[("load.cjs", "module.exports = () => require('./value.json');"), ("value.json", r#"{"value":7}"#)]);
    assert_eq!(
        script::call_plugin_launch_snapshot(&graph, "null").unwrap(),
        r#"{"value":7.0}"#
    );
...
fn commonjs_failures_queued_work_and_cache_roots_teardown_with_the_shared_limits() {
    for body in [
        ...
        let graph = snapshot("import run from './load.cjs'; export const name = 'bounded'; export function launch() { return run(); }", &[("load.cjs", &source)]);
        for _ in 0..2 {
            assert!(
...
fn caught_esm_initialization_failure_still_fails_shared_completion() {
    let graph = snapshot("import run from './load.cjs'; export const name = 'caught'; export function launch() { return run(); }", &[
...
fn commonjs_cannot_create_a_global_require_or_reopen_dynamic_imports() {
    ...
    let graph = snapshot("import run from './load.cjs'; export const name = 'closed'; export function launch() { return run(); }", &[("load.cjs", "module.exports = () => { const require = x => x + 1; return { value: require(4) }; };")]);
    assert_eq!(
        script::call_plugin_launch_snapshot(&graph, "null").unwrap(),
        r#"{"value":5}"#
    );
...
fn real_effect_option_subpath_runs_in_both_consumers() {
    let graph = native_snapshot("import * as Option from 'effect/Option'; export const name = Option.getOrElse(Option.some('effect'), () => 'absent'); export function launch() { return { value: Option.getOrElse(Option.map(Option.some(20), n => n + 2), () => 0) }; }", &[(RETROARCH, "effect")]);
    assert_eq!(
        script::eval_plugin_snapshot(&graph).unwrap(),
        r#"{"name":"effect"}"#
    );
...
fn real_pure_rand_uses_distinct_native_import_and_require_branches() {
    ...
    let graph = native_snapshot_with_files("import { generateN } from 'pure-rand/utils/generateN'; import cjs from './consumer.cjs'; export const name = 'rand'; export function launch() { return { values: generateN({ next: () => 7 }, 2), cjs: cjs.values, distinct: cjs.generateN !== generateN }; }", &[(RETROARCH, "pure-rand")], &[("consumer.cjs", consumer)]);
    assert_eq!(
        script::call_plugin_launch_snapshot(&graph, "null").unwrap(),
...
fn commonjs_is_lazy_and_preserves_cycles_replacement_and_json_identity() {
    let graph = snapshot("import load from './load.cjs'; export const name = 'closed'; export const config = { lazy: globalThis.started === undefined, require: typeof globalThis.require }; export function launch() { return load(); }", &[
...
fn require_of_esm_is_lazy_and_shares_native_namespace_identity() {
    let graph = snapshot("import load from './load.cjs'; export const name = 'closed'; export const config = { lazy: globalThis.started === undefined }; export function launch() { return load(); }", &[
...
fn full_schema_and_url_graphs_prepare_without_running_missing_platform_apis() {
    for (name, packages) in [
        ...
        let graph = native_snapshot_with_files("import load from './load.cjs'; export const name = 'prepared'; export function launch() { return { deferred: typeof load === 'function' }; }", &packages, &[("load.cjs", &loader)]);
        assert_eq!(
...
fn combined_native_graph_fits_the_unchanged_vm_guards_without_initializing_schema_or_url() {
    let graph = native_snapshot_with_files(
        "import load from './load.cjs'; import { TextEncoder } from '@kayahr/text-encoding/no-encodings'; export const name = 'combined'; export function launch() { return { deferred: typeof load === 'function', encoded: Array.from(new TextEncoder().encode('ok')) }; }",
...
fn real_package_exports_block_unexported_decoder_paths() {
    let graph = native_snapshot("import load from '@exodus/bytes/fallback/multi-byte.encodings.cjs'; export const name = 'tables'; export function launch() { const a = load(); return { same: a === load(), jis: Array.isArray(a.jis0208) }; }", &[(PROBE, "@exodus/bytes")]);
