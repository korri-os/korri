...
|---|---|
| `plugin-host/builder.nix` | Trusted composition supplies `publisher` and a closed source root. The builder copies its admitted TypeScript and metadata files and records them in `manifest.json`; `plugin.ts` is the entry point. `packages`, `files`, `services` and `requires` are not JavaScript source grants. |
...
| `services/korrid/tests/fixtures/script-packages/retroarch/package.json` and `bun.lock` | These core-owned fixtures pin the native package sources used by evaluator tests (`effect` is `4.0.0-beta.78`). The plugin repository owns the production RetroArch source. These fixture packages are not registered runtime resolution inputs. |
