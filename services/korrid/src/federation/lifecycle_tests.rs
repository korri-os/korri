...
fn binding_refreshes_existing_clients_without_rotating_port_and_stop_joins_blocked_relay() {
    ...
    let port = start_local_server_for_platform("https://portal.example", readable.path().to_str().unwrap(), private.path().to_str().unwrap(), NativePlatform::EmbeddedAndroid).unwrap();
    let _stop = Stop;
