 ...
 * something to type into even before the first key. The header already says
 * "find", so the field does not say it again. The caret is the only thing on
 * this screen that moves, which is what makes it read as the live target.
 */
export function PicoQueryField({ query }: { readonly query: string }) {
  return (
    <div className="pico-query-field">
      ...
      <span aria-hidden className="pico-query-field-caret">█</span>
    </div>
  )
}
