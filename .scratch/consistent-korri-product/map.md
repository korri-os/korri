...
- Device sleep and wake with a live session. The behavior is decided; which devices can suspend at all, and how a device with no working suspend records that limit, hangs on the boundary ticket and the hardware policy.
