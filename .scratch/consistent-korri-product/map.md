...
- [Decide the shared product and hardware boundary](issues/07-decide-product-device-boundary.md#answer): one product module owns every required part and the runtime account; a device supplies hardware facts and recorded limits only; the `window.KorriRpc` portal module is the browser path and Odin converges; the streaming host is a bundled plugin, not a product part; a supported image needs a published SD image and its signed-cache closure from one commit; one product check gates build and publication, physical acceptance grants support, with an on-device smoke run as the named future gate; the physical set is a fixed behavior list with no numeric targets; sleep tiers and the four cut-overs became child tickets 11 to 15, and all five are now resolved.
- [Decide the Odin runtime account cut-over](issues/11-decide-odin-runtime-account-cutover.md#answer): the product account is `korri` uid/gid 1000, group `korri`, home `/home/korri`, matching Odin's existing declaration; Simon excluded migration and backward-compatibility work for alpha installations; this authorizes no changes to installed data or device identity.
- [Decide the Odin browser path convergence](issues/12-decide-odin-browser-convergence.md#answer): retire the old kiosk launcher and use the shared portal shell with `window.KorriRpc`; preserve the separate Chromium transparency work; Odin supplies hardware facts while the product owns browser and focus behavior; remove the portal's dependency on Sunshine; require shared security tests and physical Odin checks for boot, rendering, input, local RPC, game-return focus, restart recovery, and operation without streaming hosting.
- [Decide the streaming host plugin boundary](issues/13-decide-streaming-host-plugin-boundary.md#answer): the plugin host widens what it admits in a native unit, and each plugin's approval binds the exact units, directives, devices, and capabilities it requested; the streaming host is in every current device's default selection except R36TMAX, which records its missing H.264 encoder as a limit; the plugin declares its ports and the host opens exactly those on every interface, so RG353M loses its per-interface narrowing; korrid performs no encoder match, so one release for each Nix system carries every encoder and Sunshine selects at start; acceptance needs a passing product check with the plugin removed, clean removal with storage reclaimed, an approval report that names the widened authority, a real pair-and-stream on an enabled device, and a recorded encoder result for each device.
- [Decide RG35XXSP product adoption](issues/14-decide-rg35xxsp-product-adoption.md#answer): RG35XXSP is a product device and its console-only composition ends; it supplies a ROCKNIX-derived Linux 7.2 kernel, because mainline describes no display engine or panel for H700 in 6.12 or 7.2; the screen, buttons, Wi-Fi and audio must work, while Bluetooth, battery percentage, clock and USB host mode may be recorded limits; 1 GB of RAM and the 640x480 panel are ordinary hardware facts, not limits; the image and cache workflow entries land with the implementation that imports the product module.
- [Decide device sleep tiers](issues/15-decide-device-sleep-tiers.md#answer): a device declares each sleep state it has instead of holding one rank, because hibernation needs no firmware suspend path and can work where deep sleep is broken; the states are light sleep, deep sleep, and hibernation, and hibernation is named but deferred; Korri declares the states in Nix and raises them by hand, with no test gating a declaration; device sleep is an optional hardware feature, so a device with no state is a supported image with a recorded limit; all six devices declare no state on day one; the power button enters the preferred state and the lid follows it, and both shut the device down cleanly where no state exists; light sleep ends in a clean shutdown after a fixed delay until hibernation lands.
...
- Hibernation. [Decide device sleep tiers](issues/15-decide-device-sleep-tiers.md#answer) names the state and defers it, including which storage may hold the memory image. The fixed delay that ends light sleep is also unchosen; it lands with the implementation.
...
- How korrid stays agnostic to the streaming host. korrid owns `korri-certificate-control` and speaks the protocol that Sunshine patch `0020` implements, and the shared host composes a seat receiver and group named for Sunshine. Ticket 13 records this as unresolved. Settle it before the cut is implemented; do not invent a generic capability name for it here.
...
- RG35XXSP's port. Ticket 14 decides the device is adopted; nothing is built. The ROCKNIX-derived kernel in `.worktree/rg35xxsp-display` is unlanded and unverified, every compositor and input fact for the board is unknown until it boots, and the device fails the shared base graphics-seat check while it enables graphics with no seat.
- Odin's browser convergence is decided in ticket 12 but remains unimplemented and unverified on hardware. Ticket 11 removes alpha account migration from scope; do not add that migration back during `/to-tickets`.
...
- Runtime-account migration and backward compatibility for existing alpha installations were excluded by Simon in [ticket 11](issues/11-decide-odin-runtime-account-cutover.md#scope-disposition). Existing data remains untouched; the decision authorizes no wipe or deployment.
