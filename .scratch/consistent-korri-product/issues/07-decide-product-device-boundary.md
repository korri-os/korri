...
Use the agreed product, hardware support policy, bundled-plugin lifecycle, and image-variant decisions. Inspect current ownership rather than assuming a new abstraction is needed. Load `/codebase-design` in addition to `/grilling` and `/domain-modeling` for this ticket.
