...
import { createRuntimeChooser as createController } from "./runtime-chooser"
import { runtimeRoutes as routes } from "./fixtures/runtime-routes"
...
function createRuntimeChooser(...args: Parameters<typeof createController>) {
  ...
  return {
    ...
    act(command: string) {
      ...
      return controller.act(action?.id.slice("runtime:".length) ?? "unpublished")
    },
  }
}
...
describe("runtime chooser", () => {
  it("cannot apply an old button to a newer route read", async () => {
    ...
    await chooser.open("wl4", "Wario Land 4", "inspect")
    await chooser.act(oldId.slice("runtime:".length))
    expect(await korrid.sessionStatus()).toEqual({ _tag: "Ok", payload: {} })
  })

  it("lists full runtime, launcher, kind, program and build identities", async () => {
    const chooser = createRuntimeChooser(client(), () => {})
    await chooser.open("wl4", "Wario Land 4", "inspect")
    const state = chooser.getSnapshot()
    if (state._tag === "Closed") throw new Error("Expected chooser")
    ...
    expect(state.saved).toEqual([
      { label: "This game", runtimeId: "missing/runtime" },
      { label: "System gba", runtimeId: "retroarch/mgba" },
    ])
  })
  ...
  it("automatically launches the sole candidate only when no choice is missing", async () => {
    ...
    const chooser = createRuntimeChooser(korrid, () => {})
    ...
    const missing = createInMemoryKorridClient({
      gameRoutes: [{ ...sole, gameRuntime: "missing/runtime" }],
    })
    const chooser2 = createRuntimeChooser(missing, () => {})
  ...
  it("reloads after a revision conflict instead of retrying the write", async () => {
    ...
    const chooser = createRuntimeChooser(korrid, () => {})
    await chooser.open("wl4", "Wario Land 4", "inspect")
    await korrid.setGameRuntime({
      scope: { _tag: "Game", id: "wl4" },
      runtimeId: "retroarch/mgba",
      expectedRevision: "g1",
    })
    ...
    expect(result._tag === "Ok" && result.payload.gameRuntime).toBe("retroarch/mgba")
  ...
  it("saves and clears a system choice without changing the game choice", async () => {
    ...
    expect(result._tag === "Ok" && result.payload.systemRuntimes.gba).toBe("retroarch/mgba-nightly")
    expect(result._tag === "Ok" && result.payload.gameRuntime).toBe("missing/runtime")
    await chooser.act("clear-system:0")
    result = await korrid.gameRoutes("wl4")
    expect(result._tag === "Ok" && result.payload.systemRuntimes).toEqual({})
  })
  ...
  it("preserves a missing saved runtime and launches once without writing it", async () => {
    const korrid = client()
    const chooser = createRuntimeChooser(korrid, () => {})
    await chooser.open("wl4", "Wario Land 4", "inspect")
    expect(chooser.getSnapshot()._tag).toBe("Stale")
  ...
  it("remembers and clears the game choice with the current revision", async () => {
    ...
    await chooser.act("game:1")
    let result = await korrid.gameRoutes("wl4")
    expect(result._tag === "Ok" && result.payload.gameRuntime).toBe("retroarch/mgba-nightly")
    await chooser.act("clear-game")
    result = await korrid.gameRoutes("wl4")
    expect(result._tag === "Ok" && result.payload.gameRuntime).toBeUndefined()
  })
