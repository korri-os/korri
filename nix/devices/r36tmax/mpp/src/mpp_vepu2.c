...
static int vepu_px30_run(struct mpp_dev *mpp, struct mpp_task *mpp_task)
{
	mpp_iommu_flush_tlb(mpp->iommu_info);
	return vepu_run(mpp, mpp_task);
}

static int vepu_px30_set_grf(struct mpp_dev *mpp)
{
	return mpp_set_grf(mpp->grf_info);
}
