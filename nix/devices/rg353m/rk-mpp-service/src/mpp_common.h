...
/*
 * The vendor tree asks its private Rockchip PM-domain driver to idle the bus
 * around a codec reset. Mainline keeps the RKVENC power domain active through
 * runtime PM but does not export that private handshake. Keep the reset
 * sequence local to the encoder clocks/resets for this narrow port.
 */
