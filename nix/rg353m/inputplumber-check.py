...
"""Validate the RG353M button map against InputPlumber's installed wire schemas."""
import json
...
def load(relative, schema):
    value = yaml.safe_load((root / relative).read_text())
    jsonschema.validate(value, json.loads((schemas / schema).read_text()))
    return value

profile = load('devices/01-rg353m.yaml', 'composite_device_v1.json')
assert profile['matches'] == [{'udev': {'sys_path': '/sys/firmware/devicetree/base', 'attributes': [{'name': 'model', 'value': 'Anbernic'}, {'name': 'compatible', 'value': 'anbernic,rg353p*'}]}}]
assert profile['source_devices'] == [{'group': 'gamepad', 'evdev': {'name': 'gpio-keys-control', 'phys_path': 'gpio-keys/input0', 'handler': 'event*'}, 'capability_map_id': 'rocknix_rg353m_map'}]
assert profile['maximum_sources'] == 1
assert profile['options']['auto_manage'] is True
assert profile['target_devices'] == ['xb360']
capabilities = load('capability_maps/rocknix_rg353m_map.yaml', 'capability_map_v2.json')
...
expected = {
    'BTN_SOUTH': 'South', 'BTN_EAST': 'East', 'BTN_NORTH': 'North', 'BTN_WEST': 'West',
    'BTN_START': 'Start', 'BTN_SELECT': 'Select', 'BTN_TL': 'LeftBumper', 'BTN_TR': 'RightBumper',
...
print('RG353M schema and all 16 physical button mappings passed')
