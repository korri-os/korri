		  ...
		  struct platform_device *pdev)
{
	...
	/*
	 * RKVENC-v1 register fields carry 32-bit IOVAs. Refuse mappings that
	 * could be silently truncated by the hardware.
	 */
	ret = dma_set_mask_and_coherent(dev, DMA_BIT_MASK(32));
	if (ret) {
		dev_err(dev, "failed to constrain DMA to 32 bits: %d\n", ret);
		return ret;
	}
