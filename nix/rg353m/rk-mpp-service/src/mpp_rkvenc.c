...
#ifdef CONFIG_ROCKCHIP_MPP_DEVFREQ
...
static int rkvenc_init(struct mpp_dev *mpp)
{
	...
	/*
	 * Set the vendor-safe default rates. Runtime DT overlays do not run the
	 * boot-time assigned-clock defaults, so apply them when the driver probes.
	 */
	mpp_set_clk_info_rate_hz(&enc->aclk_info, CLK_MODE_DEFAULT, 297 * MHZ);
	mpp_set_clk_info_rate_hz(&enc->core_clk_info, CLK_MODE_DEFAULT, 297 * MHZ);
	mpp_clk_set_rate(&enc->aclk_info, CLK_MODE_DEFAULT);
	mpp_clk_set_rate(&enc->core_clk_info, CLK_MODE_DEFAULT);
	...
	/* Use mainline's generic IOMMU fault reporting and task timeout path. */
